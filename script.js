  ...
  if (dice_positions[0].src === diceImageBlank) {
    ...
    switch (true) {
      ...
      case freqObj[`${freqObjKeyNamesArr[0]}`] === 6:
        ...
        break;
      // Two triplets
      case freqObj[`${freqObjKeyNamesArr[0]}`] === 3 &&
        freqObj[`${freqObjKeyNamesArr[1]}`] === 3:
        points += 2500;
        break;
      // 4-o-kind and a pair - two ways 4&2 or 2&4
      case freqObj[`${freqObjKeyNamesArr[0]}`] === 4 &&
        freqObj[`${freqObjKeyNamesArr[1]}`] === 2:
        points += 1500;
        break;
      case freqObj[`${freqObjKeyNamesArr[0]}`] === 2 &&
        freqObj[`${freqObjKeyNamesArr[1]}`] === 4:
        points += 1500;
        break;
      // three Pairs
      case freqObj[`${freqObjKeyNamesArr[0]}`] === 2 &&
        freqObj[`${freqObjKeyNamesArr[1]}`] === 2 &&
        freqObj[`${freqObjKeyNamesArr[2]}`] === 2:
        points += 1500;
        break;
      // straight 1-6
      ...
      default:
        for (let i = 0; i < freqObjKeyNamesArr.length; i++) {
          switch (true) {
            ...
            case freqObjKeyNamesArr[i] == 1 &&
              freqObj[`${freqObjKeyNamesArr[i]}`] < 3:
              points += freqObj[`${freqObjKeyNamesArr[i]}`] * 100;
              continue;
            // fives freq less than three
            case freqObjKeyNamesArr[i] == 5 &&
              freqObj[`${freqObjKeyNamesArr[i]}`] < 3:
...
const dieLeadToPointsBoolean = {
  threeOfKind: (x) => {
    for (let i = 0; i < dice.length; i++) {
      if (
        dice[i] === x &&
        document.getElementById(`dice-pos${i + 1}`).classList[1] === "select" &&
        document.getElementById(`dice-pos${i + 1}`).classList[2] !== "lock"
      ) {
...
for (let i = 0; i < dice_positions.length; i++) {
  dice_positions[i].addEventListener("click", () => {
    if (
      dice_pos1.src.endsWith("blank_dice.png") === false &&
      dice_positions[i].classList[2] !== "lock"
    ) {
